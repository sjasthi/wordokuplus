...
function get_quote($quote_id) {
	try {
		...
		$stmt = $pdo->prepare("SELECT word FROM words_list where lenght=:quote_id");
		$stmt->bindParam(':lenght', $quote_id);
		$stmt->execute();
 ...
 */
function create_quote($word, $length) {
	try {
		// connect to the database
		$pdo = pdo_connect_to_db();

		$stmt = $pdo->prepare("INSERT INTO words_list (word, length)
			VALUES (:word, :length)");
...
function get_recent_quotes($quotes_to_show) {
    try {
		...
		$stmt = $pdo->prepare("SELECT id, word, length FROM words_list ORDER BY id DESC LIMIT " . $quotes_to_show);
        $stmt->execute();
...
function get_quotes() {
    try {
		...
		$stmt = $pdo->prepare("SELECT * FROM words_list");
        $stmt->execute();
